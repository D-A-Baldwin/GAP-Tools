...
IsTorsionFreeSpaceGroup := function(spacegroup_gens)
	...
	local n, gens_group, gen, genslength, transvec, i, trans_elems, elems, identity_pos,
	fixpts;
	
	n := Length(spacegroup_gens[1]);
	
	gens_group := [];
	for gen in spacegroup_gens do
	...
	for i in [1..genslength] do
		Append(transvec, TransposedMat(spacegroup_gens[i])[n]{[1..n-1]});
	od;
